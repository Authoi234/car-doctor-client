    ...
    return (
        <div id={`slide${id}`} className="carousel-item relative w-full">
            ...
            <div className="absolute flex justify-start transform -translate-y-1/2 w-2/5 left-24 top-2/3">
                <button className="btn btn-warning mr-5 active:scale-90 hover:brightness-125 duration-500">Discover More</button>
                <button className="btn btn-outline btn-warning">Latest Project</button>
            </div>
            <div className="absolute flex justify-end transform -translate-y-1/2 left-5 right-5 bottom-0">
                <a href={`#slide${prev}`} className="btn btn-circle bg-slate-800 text-white border-0 mr-5">❮</a>
                <a href={`#slide${next}`} className="btn btn-circle bg-orange-600 text-white border-0">❯</a>
            </div>
        </div>
    );
};
